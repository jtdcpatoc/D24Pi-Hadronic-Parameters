/**
 * This is code that creates N number of flat phase space events, calculates the
 *bin number under the KKpipi binning scheme used by Daniel Cervenkov and Martin
 *Tat, AND the amplitude for each event.
 *
 * No input.
 *
 * Output is a ROOT file which stores the real and imaginary parts of the
 *amplitudes, the 5D phase space point and the bin number.
 *
 *
 **/
...
int main() {
  ...
  for (Int_t n = 0; n < nevents; n++) {
    ...
    // TGenPhaseSpace is independent of particle ordering (Martin). Keep the
    // ordering the same as the one for the MC to avoid confusion.
    ...
    if (TMath::IsNaN(Damp_real) || TMath::IsNaN(Damp_imag) ||
        TMath::IsNaN(DBARamp_real) || TMath::IsNaN(DBARamp_imag)) {
      return 0;
    }
    ...
    BinNumber = static_cast<int>((StrongPhaseDiff + TMath::Pi()) /
                                 (2 * TMath::Pi() / number_of_bins)) +
                1;
