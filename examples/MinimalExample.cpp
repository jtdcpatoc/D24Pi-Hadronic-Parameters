...
int main(int argc, char *argv[]) {
  ...
  for(std::size_t n = 0; n < NumberIterations; n++) {
    ...
    // Particle ordering: pi+ pi+ pi- pi- (however, TGenPhaseSpace does not care for particle ordering so this is just arbitrary)
    std::array<TLorentzVector, 4> Daughters;
    for(std::size_t i = 0; i < 4; i++) {
      Daughters[i] = *PhaseSpace.GetDecay(i);
    }

    // Calculate the five variables that parameterise the D decay (Can Copy)
    ...
    // Create hyperpoint and determine bin number (Can Copy)
