/**
 * This code calculates the  Hadronic parameters AND the CP-even fraction from a given binning scheme based of code from Martin Tat. 
 * 
 * The input is a ROOT file (produced from either KKPiPiFlat.cpp or GenerateFlat.cpp) where each event is given a bin number from the given scheme. The ROOT file should have the real and imaginary parts of the amplitude. 
 * 
 * Output are the hadronic parameters; given in the terminal. 
 * 
 * Note: At the moment, the bin number is hard-coded (line 91). 5 for the BES3 schemes.
 * 
...
//____________________________________________//
  ...
  for(Int_t n = 0; n < nevents; n++) {
    ...
    F_P_nom += ( pow(Damp_Mag,2) + pow(DBARamp_Mag,2) + 2*Damp_Mag*DBARamp_Mag*TMath::Cos(StrongPhaseDiff))*EventWeight;
    ...
    m_c[BinIndex] += TMath::Sqrt(std::norm(Damp)*std::norm(DBARamp))*TMath::Cos(StrongPhaseDiff)*EventWeight;
    
    m_s[BinIndex] += TMath::Sqrt(std::norm(Damp)*std::norm(DBARamp))*TMath::Sin(StrongPhaseDiff)*EventWeight;
