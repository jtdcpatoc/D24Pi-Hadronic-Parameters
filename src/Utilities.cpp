...
namespace Utilities {
  ...
  double getPhi(std::array<TLorentzVector, 4> daughters) {
    ...
    const TVector3 Avect = (daughtersVect[0] + daughtersVect[2]).Unit();
    const TVector3 Bvect = (daughtersVect[1] + daughtersVect[3]).Unit();
  
    // Find the unit vector that is normal to A1 and A2

    const TVector3 normAvect = (daughtersVect[0].Cross(daughtersVect[2])).Unit();

    // Find the unit vector that is normal to B1 and B2

    const TVector3 normBvect = (daughtersVect[1].Cross(daughtersVect[3])).Unit();
